...
post '/gateway' do
  ...
  elsif (message.split(' ') & faq_init).any?
    answer = fetch_faq_answer(message)
    if answer == "speaker"
      begin_individual_response(message)
    elsif answer == []
       bingo(message)
    elsif answer == "test"
      ...
      code = [code1, code2]
      respond_normal(code.sample)
    elsif answer == "question"
      respond_normal(fetch_question)
    elsif answer == "bathroom"
      respond_bathroom
    else
      respond_message(answer)
    end
  elsif (message.split(' ') & joke_init).any?
    respond_message(fetch_jokes)
  elsif (message.split(' ') & compliment).any?
    respond_normal(fetch_compliment)
  elsif (message.split(' ') & love).any?
    respond_normal(fetch_love)
  elsif (message.split(' ') & snap).any?
    respond_normal(fetch_snap)
  elsif (message.split(' ') & lol).any?
    respond_normal(fetch_lol)
  elsif (message.split(' ') & wishes).any?
    respond_normal(fetch_wishes)
  elsif (message.split(' ') & @individual_list_1).any? || (message.split(' ') & @individual_list).any?
    begin_individual_response(message)
  elsif (message.split(' ') & schedule_list).any?
    process_schedule_data
    respond_message(["The Schedule for the day:", @final_schedule.join("\n")])  
  else
    bingo(message)
  end
